...
void loop() {
  if (millis() > timestamp) { // Is it time yet?
    timestamp += 50;         // Set the next time routine will run. 500 ms because the lowest frequency is 2 Hz, which is a half second.
    int pot = analogRead(A0);            // Read the potentiometer connected to analog pin A0 to control alternating flashing speed.
    int val = map(pot, 0, 1023, 0, 23); // Convert pot analog values to a range from 2 to 40 Hz.
    int freq = note(val);
